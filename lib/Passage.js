    ...
    result = result.replace(
      ...
      function (match, display, params, target) {
        ...
        if (
          matchSend === null &&
               matchCC === null &&
               matchUrgent === null && matchType === null
        ) {
          paramsArray.push(params);
        } else {
          if (matchSend !== null) paramsArray.push(`sendTo:${matchSend.slice(1).join(',')}`);

          if (matchCC !== null) paramsArray.push(`cc:${matchCC.slice(1).join(',')}`);
