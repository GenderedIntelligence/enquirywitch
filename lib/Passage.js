  ...
  render(source) {
        ...
        switch (type) {
          ...
          case "text-long":
            return (
              ...
              text +
              '</strong></p><p class="input-label"><textarea class="input-text-long" required rows="4" data-bind="' +
              variable +
    ...
    result = result.replace(
      ...
      function (match, display, params, target) {
        ...
        const matchSend = paramRegexps[0].exec(params);

        const matchCC = paramRegexps[1].exec(params);

        const matchUrgent = paramRegexps[2].exec(params);

        const matchType = paramRegexps[3].exec(params);
        ...
        if (
          ...
          if (matchSend !== null)
            paramsArray.push(`sendTo:${matchSend.slice(1).join(",")}`);

          if (matchCC !== null)
            paramsArray.push(`cc:${matchCC.slice(1).join(",")}`);

          if (matchUrgent !== null) paramsArray.push("urgent");

          if (matchType !== null) paramsArray.push(`type:${matchType[1]}`);
        }
    ...
    /* SUBMITSUMMARY  */
    result = result.replace(/\[SUMMARY\]/g, function (match, target) {
        ...
        formdata
          .map((form) => {
            if (Object.keys(form)[0] === "upload") {
              ...
              return (
                "<tr><td><strong>" +
                Object.keys(form)[0].split("_").join(" ").toUpperCase() +
                "</strong></td><td>" +
